...
import math
from transformers import AutoModelForCausalLM, AutoTokenizer, pipeline, AutoModelForSeq2SeqLM, GPTNeoXForCausalLM, OPTForCausalLM
...
##########################################################################################
def ask_chatgpt(text, target, stance, text_last,df_examples,args):
    def ChatEngine(prompt,instruction):
         #zcy8863
        # openai.api_key="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" #czhao43
    ...
    # prompt="""Rephrase the following text to subtly express the same stance towards the specified target. The rephrasing should aim to convey the message in a manner that is not straightforward for stance detection algorithms to classify. Consider using synonyms, similar expressions, indirect language, metaphors, or summarizing the sentiment in a nuanced way that retains the original stance towards the target but changes the sentence structure and vocabulary significantly. Please ensure the rephrased text remains coherent and can be understood by a human reader as maintaining the original stance towards the given text without directly mimicking the original text's style or terminology.
    prompt="""In this task, we'll delve into understanding the stance and the semantic meanings conveyed in the provided text, focusing on its correlation with the specified target. By breaking down the analysis into clear, logical steps, we aim to elucidate how the text's language and structure communicate its stance towards the target. This detailed examination will not venture into rephrasing but will instead concentrate on dissecting and interpreting the original content.

        1.Analyze the Stance Towards the Target: Identify and analyze the stance the text takes towards the specified target. Is the stance favor, against, or neutral? Determine the nature of the sentiment being expressed towards the target.
        2.Semantic Analysis of the Text: Analyze the semantic layers within the text. What words or phrases are pivotal in conveying the stance? How do these elements interplay to express favor, against, or neutral towards the target?
        3.Key Elements Influencing Stance Expression: Identify key elements and linguistic features in the text that influence how the stance is expressed. Consider tone, word choice, and any figurative language used. How do these contribute to the overall stance orientation?
        4.Correlation Between Text and Stance: Examine how the identified semantic elements and linguistic features correlate with the expressed stance. How effectively does the text communicate its position regarding the target? Are there subtle cues or overt statements that make this stance evident?

        Please apply this analytical process to the following text, target and the stance correlation, focusing on the semantic meaning and the stance correlation:

        Original Text: {}
        Target: {}
        Stance: {}""".format(text, target, stance)
...
# print(bk)
idx = 0
for elem in to_annotate_examples:
  idx+=1
  # if idx>10:
  #   continue
  # if idx<300:
  ...
  max_try = 0 # does not matter here as step1 is to generate cot
  cnt_tries = 0
  results = []
  ...
  while True:
    ...
    if args['model']=='chatgpt':
      inp = ask_chatgpt(elem['text_raw'],' '.join(elem['target']),prediction,text_last,df_in_context_examples,args)
      print(100*"!")
      print("inp:",inp)
      if "3):" in inp:
          inp = inp.split("3):")[-1]
